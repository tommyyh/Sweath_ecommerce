...
import SaleSortOption from './Components/SaleSortOption';
...
const Sale = () => {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      const res = await axios.post(`/products/on-sale/${title}`, filter);
      ...
      {!category ? (
        ...
        <>
          <HelmetProvider>
            {/* Head */}
            <Helmet>
              <title>Sweath: Sale - {category.title}</title>
            </Helmet>
          </HelmetProvider>
          {/* Body */}
          <section>
            <div className='sale_title'>
              <H2 title='PHONES / PHONES' />
              <div className='sale_title_group'>
                <h1>
                  SALE - {category.title.toUpperCase()} {category.length}
                </h1>
                <FaSortAmountDown
